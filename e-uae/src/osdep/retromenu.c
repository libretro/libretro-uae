...
//memory
	char tmp[256];
	sprintf(tmp, "%s %s mem ch=%s fa=%s bo=%s\0", 
		cpu_name,
		chipset,
...
void update_misc_menu(void) {
	...
	//sprintf(string, " %i  %i \0",  currprefs.jport0, currprefs.jport1);
...
//				name = fsel_open(rpath, currprefs.df[floppyIdx], previousDiskName, 0);
	...
	//				disk_insert(menu_pos, name);
		...
		/* RESET */
		case MPOS_RESET :{ uae_reset(0);retro_uae_reset(0);
			return 1;}
	}
	return 0;
}
...
void enter_options(void) {
	...
	while (!exit) {
		...
		event = update_input_gui();//handle_options_events();
