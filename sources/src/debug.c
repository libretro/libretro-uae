...
static uae_u32 readint_size (TCHAR **c, int *size)
{
	return readnum (c, size, '!');
}
static uae_u32 readhex_size (TCHAR **c, int *size)
{
	return readnum (c, size, '$');
}
		...
		if (more_params (c)) {
			...
			if (more_params (c)) {
				if (_totupper (**c) == 'M') {
					...
					mwn->val = readhex_size (c, &mwn->val_size);
					mwn->val_enabled = 1;
				}
			}
		}
