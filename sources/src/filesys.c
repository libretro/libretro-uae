...
int get_filesys_unitconfig (struct uae_prefs *p, int index, struct mountedinfo *mi)
{
	...
	if (!ui) {
		...
		if (uci->ci.type == UAEDEV_DIR) {
			...
			int err = hdf_open_2 (&ui->hf, filepath);
			if (err <= 0) {
				mi->ismedia = false;
				mi->ismounted = true;
				mi->error = err;
...
static uae_u8 *restore_key (UnitInfo *ui, Unit *u, uae_u8 *src)
{
	...
	if (a) {
		...
		if ((u->volflags & (MYVOLUMEINFO_ARCHIVE | MYVOLUMEINFO_CDFS)) || my_existsfile (pn)) {
			 ...
			 * "missing". fs_openfile() already knows how to resolve such
			 * units via the archive volume/CDFS, exactly like normal
			 * (non-restore) file access does, so just use it directly
			 * instead of falling
			 * into the dummy-file path below. */
